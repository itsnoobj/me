...
The same applies to management overhead. Every process you add, every approval gate, every status meeting, every "quick sync" — it's extraneous load. It's weight on the team's working memory that doesn't help them solve the problem. If you've read [Your Agents Are Running. So Why Are You Still Exhausted?](/notes/cognitive-load-agentic-ai), you'll recognise this — even with AI doing the heavy lifting, the *coordination* load can still crush you.
...
Donella Meadows would recognise this immediately — it's a **reinforcing feedback loop**. The same kind of coordination failure that makes [everyone busy but nothing ship faster](/notes/why-everyones-busy-but-nothing-ships-faster).
...
| Optimise for preventing all mistakes | Optimise for recovery speed — [trust + guardrails > gatekeeping](/notes/governance-theatre) |
...
| Let the [titled lead run everything](/notes/everybody-is-junior-in-something) even when someone else knows the domain | Leadership follows expertise, not org charts |
